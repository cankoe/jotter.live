...
function mentionCompletion(contacts) {
    return async (context) => {
        ...
        const options = filteredContacts.slice(0, 10).map(contact => ({
        ...
        if (query.length > 0 && !filteredContacts.some(c => c.handle.toLowerCase() === query.toLowerCase())) {
            options.push({
                label: query,
...
function hashtagCompletion(existingTags) {
    return async (context) => {
        const beforeCursor = context.state.sliceDoc(Math.max(0, context.pos - 50), context.pos);
        ...
        const filteredTags = existingTags.filter(tag => tag.toLowerCase().includes(query));
        const options = filteredTags.slice(0, 10).map(tag => ({
            label: tag,
            type: "text",
            apply: (view, completion, from, to) => {
                const insert = `${tag} `;
...
export function createCaptureAutocomplete(contacts = [], existingTags = []) {
...
export function triggerAutocomplete(view) {
